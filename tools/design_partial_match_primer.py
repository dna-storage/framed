...
if __name__ == "__main__":
    ...
    parser.add_argument('--Tmdelta',type=int,dest="tmdelta",action="store",default=5, help="Melting temperature delta")
        ...
        for p in primers:
            ...
            for j in range(0,perStep):
                while True:
                    ...
                    if checkTm(m,(Tm+Tmdelta-1,Tm+Tmdelta+1)):
    ...
    if args.o != None:
        ...
        out.close()
    #else:
    print "Input: ",args.primer,getTm(args.primer),GC(args.primer)
    for p in bad_primers:
        print p,"\t",getTm(p),"\t",hamming_distance(p,args.primer),GC(p)
