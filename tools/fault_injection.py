...
from danstorage.system.formats import *
import logging
import numpy as np
import sys
import os
...
def _monte_kernel(monte_start,monte_end,args): #function that will run per process
    ...
    primer5 = "CAGGAGAATGCCTTCCTAGG"
    primer3=  "CCTCGGTTCTTCTTGACCAG"
    ...
    if os.path.exists(args.enc_params):
        with param_file as open(args.enc_params,'rb'):
    ...
    write_dna = DNAFile.open(None,"w",primer5=primer5,primer4=primer4,fsmd_abbrev=args.arch,**encoding_params,fi=True)
    ...
    fault_environment =  Fi_Env(args.strand_distribution, args.fault_model,write_dna.strands,fault_rate=args.fault_rate,
                                ...
                                fault_file=args.fault_rate_file)
    
    for sim_number in range(monte_start,monte_end+1):
        logging.info("Monte Carlo Sim: {}".format(sim_number))
        fault_environment.Run()

        read_dna = DNAFile.open(fault_environment.get_strands(),"rf",primer5,primer3,fsmd_abbrev='FSMD')
...
def _monte_parallel_wrapper(monte_start,monte_end,args): #wrapper for parallelization
    ...
    return _monte_rate_kernel(monte_start,monte_end,*args)
...
def run_monte(args):
    ...
    pickle_fd=open(stats_pickle_path,'w+')
    dist=[]
    ...
    logging.info("Running Monte Carlo sim for error rate {}".format(error_rate))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--faulty_count',dest="faulty",action="store",default=10,type=int,help="range to sweep the number of faulty/missing strands")
    parser.add_argument('--fail_count',dest="fails",action="store",default=2,type=int,help="range to sweep the number of nucleotide errors ")

    parser.add_argument('--fault_rate',dest="rate",action="store",default=0.001,type=float,help="range to sweep th")
