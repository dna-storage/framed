...
class data_helper:
    ...
    def plot_pmf(self, read_randomizer):
        mean=read_randomizer.get_mean()
        var=read_randomizer.get_var()
        x=np.linspace(start=0,stop=mean*3,dtype=np.int32,num=mean*3)
        y=[read_randomizer.pmf(_) for _ in x ]
        pmf_fig,pmf_ax=plt.subplots(nrows=1,ncols=1)
        pmf_ax.plot(x,y,label="pmf")
        pmf_ax.set_xlabel("Strand Read Count")
        pmf_ax.set_ylabel("Probability")
        pmf_ax.set_title("PMF of Simulated Read Distribution")
        pmf_fig.savefig(self._out_file_directory+"/pmf.png")
