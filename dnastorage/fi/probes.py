...
class CodeWordErrorRateProbe(BaseCodec,Probe):
    ...
    def __init__(self,probe_name="",CodecObj=None):
        ...
        if probe_name=="":
            self._name = "codeword_probe_{}".format(probe_id)
        else:
            self._name = probe_name
        self._initial_codeword_attr = "{}_init_codewords".format(self._name)
        self._total_error_rate_key = "{}_total_errors".format(self._name)
        self._strands_seen_key = "{}_total_strands".format(self._name)
        probe_id+=1
    ...
    def decode(self,s):
        ...
        if not isinstance(s,BaseDNA): #only analyze strands
            return
        assert hasattr(s,self._initial_codeword_attr)
